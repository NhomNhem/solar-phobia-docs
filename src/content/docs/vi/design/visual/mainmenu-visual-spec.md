---
title: 'MainMenu Đặc tả thiết kế hình ảnh — Solar Phobia'
description: 'Bản dịch tiếng Việt cho MainMenu Đặc tả thiết kế hình ảnh — Solar Phobia.'
---
...
> **Ngày**: 2026-05-08  
> **Phase Bối cảnh**: Day palette only (MainMenu exists outside game loop)
...
| **Selection/Active** | Ochre Gold | `#D4A03A` | Hover trạng thái, selected items |
...
- Tiêu đề glow: Ochre Gold `@ 30%` opacity
...
- Tab không hoạt động: Canvas Cream `#D9CDB8`
- Tab đang hoạt động: Burnt Sienna `#C45A30`
...
> **Rule**: Every color-coded trạng thái phải have a shape/icon backup for colorblind accessibility
...
|---------|------|----------|
| **Main Tiêu đề** | Rosa Cleaner | System sans-serif |
...
| **Phiên bản Number** | Caveat | System cursive |

### 3.2 Font Sizes

| Element | Size | Weight | Line Height |
|---------|------|--------|-------------|
| **Main Tiêu đề** | 72px | Bold | 1.0 |
...
| **Credits Tiêu đề** | 32px | Bold | 1.2 |
| **Credits Section** | 18px | Normal | 1.6 |
| **Credits Names** | 16px | Normal | 1.4 |
| **Phiên bản Number** | 12px | Normal | 1.0 |

### 3.3 Text Effects

**Tiêu đề Shimmer** (continuous):
...
**Triển khai Note**: Use multiple box-shadow layers to achieve the "pool" effect where shadow appears to bleed outward like wet paint.
...
- Border nên not be perfectly uniform — slight variation in thickness
- Corners slightly rounded (8px radius) to feel organic

### 5.4 Tiêu đề Glow/Shimmer
...
|---------|-------|----------|--------|---------------|-------------|
| Tiêu đề | 0ms | 400ms | ease-out | opacity: 0, translateY: 20px | opacity: 1, translateY: 0 |
...
| Phiên bản | 400ms | 400ms | ease-out | opacity: 0 | opacity: 1 |
...
### 7.5 Tiêu đề Animation (Continuous)
...
| Settings | First control in đang hoạt động tab | Consistent entry point |
...
The MainMenu background nên evoke the "watercolour on paper" aesthetic without competing with the menu UI. It nên feel like looking at a painted surface — textured, warm, and alive — without being a detailed illustration.
...
| Color | Paper White with subtle warmth | Bối cảnh-dependent (day/night) |
...
- [ ] All trạng thái match Section 8 specifications exactly
- [ ] Hand-drawn border effect applied to buttons
- [ ] Paper texture applied to background
- [ ] Tiêu đề shimmer animation implemented

---

## 11. Accessibility Color Contrast (WCAG AA)

| Element | Foreground | Background | Ratio | Trạng thái |
|---------|------------|------------|-------|--------|
| Default button text | #1A1412 | #F5F0E8 | 10.3:1 | ✓ Pass |
| Hover button text | #D4A03A | #F5F0E8 | 4.1:1 | ✓ Pass |
| Disabled text | #D9CDB8 @ 50% | #F5F0E8 | 2.1:1 | ⚠ May fail - use darker bị tắt color |
| Settings labels | #1A1412 | #F5F0E8 | 10.3:1 | ✓ Pass |
| Focus ring | #D4A03A | #F5F0E8 | 4.1:1 | ✓ Pass |

**Correction needed**: For bị tắt text, use `#8A8578` (darker Canvas Cream) instead of 50% opacity to ensure 4.5:1 minimum contrast.
...
4. Tham chiếu in USS via `font-family: Rosa Cleaner, sans-serif;`
...
### 12.3 Animation Triển khai

Use Unity's `UIStyleAnimation` or custom coroutines:
- Entry animations: Play on `OnAnimationStart`
- Hover/press: USS pseudo-classes xử lý automatically
- Tiêu đề shimmer: `RegisterCallback<GeometryChangedEvent>` +DOTween or custom animator
- Screen transitions: View controller điều phối
