---
title: 'Đặc tả UX menu chính — Solar Phobia'
description: 'Bản dịch tiếng Việt cho Đặc tả UX menu chính — Solar Phobia.'
---
...
**Screen Purpose**: Primary entry point for the game. Displays title, navigation options, and handles save/load trạng thái detection.

**Platform**: Windows PC (keyboard/mouse + gamepad)

**Visual Style**: "Hand-painted watercolour with fire damage" — matches Art Bible. Day-phase styling for MainMenu (as the player is in a trạng thái of "before choice").

**Phase Bối cảnh**: MainMenu is phase-agnostic — it displays the Day palette regardless of where the player last exited, since it's a menu hệ thống outside the game loop.
...
|--------|--------|--------|
| Tiêu đề Screen | MainMenu | Fade in from black on boot |
...
|--------|---------|-------------|
| New Game | Select New Game | `PhaseState: Dịch vụ ban ngày` |
...
**Layout Notes**:
- Tiêu đề centered horizontally, top 25% of screen
- Menu buttons centered, left-aligned text, 60% down screen
- Phiên bản number bottom-right corner, small text
...
| Toggle checkboxes | A Button | X Button | Toggle trạng thái, checkmark appears |
...
## 5. Accessibility Yêu cầu

### 5.1 Minimum Text Sizes

| Element | Minimum Size | Recommended Size |
|---------|--------------|------------------|
| Headings (H1 - Tiêu đề) | 36px | 48px |
| Headings (H2 - Section) | 24px | 32px |
| Body text | 16px | 18px |
| Button labels | 18px | 20px |
| Small/caption text | 14px | 16px |

**UIToolkit Triển khai**: Use `UITextElement` and scale via USS `font-size`. Support 100%-200% text scaling via `UI.Settings.scaleFactor`.
...
| Hover trạng thái | Ochre Gold #D4A03A | Paper White #F5F0E8 | 4.1:1 ✓ |
...
**Critical**: Never rely solely on color. Every color-coded trạng thái phải have a shape/icon backup:
...
- **Visibility**: All focus trạng thái phải meet contrast requirements
...
**Triển khai**: Root container applies `scale` transform based on `PlayerPrefs.GetFloat("UIScale", 1.0f)`.
...
**Triển khai**: Use `ILocalizationService` (project's localization abstraction).
...
| Phiên bản number | Build info | `MAINMENU_VERSION` (format: "v{0}") |
...
**Tiêu đề and Menu Items**:
- **Staggered fade-in**: Tiêu đề (0ms) → first button (100ms) → subsequent buttons (+50ms each)
- **Duration**: 400ms per item
- **Easing**: ease-out
- **Initial trạng thái**: opacity 0, translateY 20px
- **Final trạng thái**: opacity 1, translateY 0
...
| **Disabled** | 50% opacity, grayscale, no tương tác | No input accepted |
...
| **Disabled** | Gray track, no tương tác |
...
- Transition to `PhaseState.Dịch vụ ban ngày`
- Initialize fresh save file

### 10.4 Continue Behavior

- Load save file data
- Resume from saved phase trạng thái
- Transition to appropriate scene

---

## 11. Technical Ghi chú triển khai
...
- `.menu-button--bị tắt` — Disabled trạng thái
- `.menu-button--hover` — Hover trạng thái (via USS pseudo-class)
- `.menu-button--focused` — Gamepad focus (via USS pseudo-class)

### 11.3 Event System

The UI NEVER modifies game trạng thái directly. All user actions fire event:
...
### 12.1 Functional Yêu cầu

- [ ] MainMenu displays within 1 second of game launch
- [ ] Continue button được bật only when valid save exists
...
- [ ] All text displays via localization hệ thống (no hardcoded strings)

### 12.2 Visual Yêu cầu

- [ ] Day palette applied (#F5F0E8 background, #C45A30 accents, #D4A03A lựa chọn)
...
| System | Dependency Loại | Notes |
|--------|-----------------|-------|
| `GameStateManager` | Runtime | Detect phase hiện tại for Continue |
...
| `PhaseState` enum | Data | Phase trạng thái definitions |

---

## 14. Revision History

| Ngày | Phiên bản | Changes |
|------|---------|---------|
