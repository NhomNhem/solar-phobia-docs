---
title: 'Module hiệu ứng nguyền rủa'
description: 'Bản dịch tiếng Việt cho Module hiệu ứng nguyền rủa.'
---
...
Module hiệu ứng nguyền rủa control the karma-based hazards that spawn during Phase ban đêm based on which soul was abandoned during Phase ban ngày. Each curse (Drag/Block/FakeShrine) manifests as specific environmental hazards with unique visual and audio signatures. Without this hệ thống, the abandoned soul has no mechanical consequence—choices feel arbitrary rather than meaningful.

## Player Fantasy

The player nên feel **haunted specificity**—not generic danger, but *this particular* horror because of *this particular* choice. The water trap feels like Linh's drowning. The blood net feels like Van's capture. The illusion platform feels like Minh's betrayal. The horror is personal and deserved.

## Detailed Design

### Lõi Rules

1. **Curse Loại Detection**: Receives curse_type (Drag/Block/FakeShrine) from Bộ xử lý hậu quả at night start.
...
|-------|-----------------|----------------|----------|
| Idle | Not Sinh tồn ban đêm | Sinh tồn ban đêm starts | No hazards spawned |
| CurseActive | Sinh tồn ban đêm + curse received | Night ends | Spawn and manage hazards |
| HazardTriggered | Player enters hazard zone | Hazard resolves | Apply effect (DoT/slow/collapse) |
| HazardCleared | Player exits hazard zone | Hazard không hoạt động | Remove effect |

### Interactions with Other Systems

- **Bộ xử lý hậu quả -> Module hiệu ứng nguyền rủa**: Sends curse_type payload.
- **Chạy sinh tồn ban đêm <- Module hiệu ứng nguyền rủa**: Hazards đang hoạt động during night.
- **Điều phối map và spawn <- Module hiệu ứng nguyền rủa**: Receives spawn bias for hazard placement.
- **Hệ thống phản hồi giác quan <- Module hiệu ứng nguyền rủa**: Curse affects audio/visual degradation.
...
|--------|-----------|---------------------|
| Bộ xử lý hậu quả | This depends on it | **Hard**: receives curse_type payload |
| Chạy sinh tồn ban đêm | This depends on it | **Hard**: hazards đang hoạt động during night |
| Điều phối map và spawn | This depends on it | **Soft**: spawn bias coordination |
| Hệ thống phản hồi giác quan | Depends on this | **Soft**: curse affects sensory tier |
...
|----------|-------|----------|------------|
| Nên curse intensity scale with game progress? | Game Designer | Before expansion | Open